...
bool validarFecha(Fecha f) { //No usad
...
bool validarFechasReserva(Fecha fechaReserva, Fecha fechaSalida) { //No usad
...
bool validarHoraEmbarque(int horas, int minutos) { //No usad
...
void eliminarReserva() {
    ...
    while (fread(&res, sizeof(Reserva), 1, fOriginal)) {
        if (strcmp(res.numero_reserva, nroReservaBuscada) == 0) {
            ...
            cout << "\n¿ESTA SEGURO QUE DESEA ELIMINAR ESTA RESERVA? (S/N): ";
            ...
            if (confirmacion == 'S' || confirmacion == 's') {
                ...
                // No escribir en el archivo temp (así se elimina)
                continue;
            } else {
                cout << "ELIMINACION CANCELADA POR EL USUARIO." << endl;
                // Sí escribir en el archivo temp (no eliminar)
                fwrite(&res, sizeof(Reserva), 1, fTemp);
                continue;
            }
        }
