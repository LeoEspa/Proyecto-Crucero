...
int buscarReserva(char numero[20], Reserva &res); //No usada
float calcularPrecioTotal(Reserva &r); //No usada
void consultar();
...
void incluir(){
    ...
    if (buscarReserva(nueva.numero_reserva, nueva) == 0) {
		...
		if(strcmp(nueva.soli.viaja,"Si")== 0){nueva.num_pasajeros=validar_numero(1,3,"Numero de pasajeros adicionales (1-3): ",40,14); condicion=true;}
		...
		for (i=0; i < nueva.num_pasajeros; i++) {
			...
			if(i==0 && condicion==true){
				...
				gotoxy(40, 3);cout << "DATOS DEL PASAJERO " << i+1;//Encabezado.
           		temp=validar_dosletras("Genero (M/F):",3,8,"M","F","m","f");
				if (temp == 'm' || temp== 'M'){
